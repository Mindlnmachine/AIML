...
# from plotly.subplots import make_subplots  # Removed unused import
import warnings
...
def run_analysis(df_input):
    """
    Runs the fraud detection analysis on the provided dataframe.
    """
    df = df_input.copy() # Work on a copy to avoid side effects
    ...
    if total_fraud > 0:
        ...
        with col1:
            ...
            sizes = [fraud_counts.get(False, 0), fraud_counts.get(True, 0)]
...
if uploaded_file is not None:
    try:
        ...
        st.sidebar.info("Custom dataset loaded. Click 'Run Analysis' to process.")
...
# Main button to run analysis
if st.sidebar.button("🔄 Run Fraud Detection Analysis", type="primary"):
    if df is not None:
        # Define required columns for analysis and derived features
        base_cols = ['ExpenseAmount', 'Allowance', 'WorkLogs', 'ProjectCount', 'OvertimeHours']
        
        # Check if all required base columns are present
        if all(col in df.columns for col in base_cols):
            # Add derived features if they don't exist
            if 'ExpenseRatio' not in df.columns:
                df['ExpenseRatio'] = df['ExpenseAmount'] / df['Allowance']
            if 'ProductivityScore' not in df.columns:
                df['ProductivityScore'] = (df['WorkLogs'] * df['ProjectCount']) / (df['OvertimeHours'] + 1)
            
            # Ensure other columns used for display exist, or create dummy ones
            if 'Department' not in df.columns: df['Department'] = 'Unknown'
            if 'EmployeeID' not in df.columns: df['EmployeeID'] = range(len(df))
            if 'Name' not in df.columns: df['Name'] = [f'Employee_{i}' for i in range(len(df))]
...
if 'df' in locals() and df is not None:
    st.write("Below are some initial visualizations of the loaded data before running the full analysis.")
    
    # Define columns for plotting
    plot_cols = ['ExpenseAmount', 'WorkLogs', 'ExpenseRatio', 'ProductivityScore', 'OvertimeHours', 'Allowance']
    # Filter for columns that actually exist in the dataframe
    plot_cols_exist = [col for col in plot_cols if col in df.columns]

    if plot_cols_exist:
        # Distribution plots
        st.write(f"**Distribution of Key Features**")
        
        # Create a figure with multiple subplots, arranged dynamically
        n_plots = len(plot_cols_exist)
        n_cols = 3
        ...
        st.write("This heatmap shows how different numerical features are related to each other.")
        fig_corr, ax_corr = plt.subplots(figsize=(8, 6))
        corr = df[plot_cols_exist].corr()
        sns.heatmap(corr, annot=True, fmt=".2f", cmap='coolwarm', ax=ax_corr)
        st.pyplot(fig_corr)
    else:
        st.warning("Could not find the necessary columns for plotting in the uploaded dataset.")
